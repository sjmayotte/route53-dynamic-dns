...
| `IPCHECKER`              | `string`  | No (default: `ifconfig.co`)                           | Public IP checker service. `ifconfig.co`, `ipify.org` or `custom` (see `IPCHECKER_CUSTOM_URL` below).  See note below for 'opendns'                                  |
| `IPCHECKER_CUSTOM_URL`   | `string`  | Yes if `IPCHECKER = custom`                           | URL to perform HTTP get request to in order to obtain external IP. Server is expected to return the IP as plain text and nothing else.                               |
